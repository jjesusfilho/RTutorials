...
-------------|-------------|-------------
Left Join | Keeps the entire first table, joins matching rows from the second | `left_join(x, y, by="ID")`
Right Join | Keeps the entire second table, joins matching rows from the first | `right_join(x, y, by="ID")`
...
Semi Join | keeps one row for every row in the first table that matches the second | `semi_join(x, y, by="ID")`
Anti Join | Removes rows that match in the second tables | `anti_row(x, y, by="ID")`
